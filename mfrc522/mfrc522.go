...
func NewSPI(spiPort drivers.SPI, resetPin machine.Pin, irqPin machine.Pin, configs ...configF) (*Device, error) {
	...
	raw, err := NewLowLevelSPI(spiPort, resetPin, irqPin)
	if err != nil {
		return nil, err
	}
...
func (r *Device) request() (int, error) {
	...
	_, backBits, err := r.LowLevel.CardWrite(PCD_TRANSCEIVE, []byte{0x26})
	if err != nil {
		return -1, err
	}
	if backBits != 0x10 {
...
func (r *Device) antiColl() ([]byte, error) {
	...
	backData, _, err := r.LowLevel.CardWrite(PCD_TRANSCEIVE, []byte{PICC_ANTICOLL, 0x20}[:])
...
func (r *Device) antiColl2() ([]byte, error) {
	...
	serial := []byte{PICC_ANTICOLL2, 0x20}
	check := byte(0)

	backData, _, err := r.LowLevel.CardWrite(PCD_TRANSCEIVE, serial)
	if err != nil {
		return nil, err
	}
...
func (r *Device) selectTag(serial []byte) (byte, error) {
	...
	backData, backLen, err := r.LowLevel.CardWrite(PCD_TRANSCEIVE, dataBuf)
	if err != nil {
		return 0, err
	}
...
func (r *Device) write(blockAddr byte, data []byte) error {
	...
	newData[16] = crc[0]
	newData[17] = crc[1]
	read, backLen, err = r.LowLevel.CardWrite(PCD_TRANSCEIVE, newData[:])
	if err != nil {
		return err
	}
...
func (r *Device) preAccess(blockAddr byte, cmd byte) ([]byte, int, error) {
	...
	send[2] = crc[0]
	send[3] = crc[1]
	return r.LowLevel.CardWrite(PCD_TRANSCEIVE, send)
}
